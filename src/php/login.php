...
switch($act){
	case 'add':
		...
		$sql = "SELECT COUNT(*) FROM kaola WHERE username='{$user}'";
		...
		$sql = "INSERT INTO kaola (username,password) VALUES('{$user}','{$pass}')";
	...
	case 'login':
		$sql = "SELECT COUNT(*) FROM kaola WHERE username='{$user}' AND password='{$pass}'";
		$res = mysqli_query($conn,$sql);
